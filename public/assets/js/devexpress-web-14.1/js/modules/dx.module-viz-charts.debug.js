    ...
    (function($, DX, undefined) {
        ...
        var correctAfterSynchronize = function(axesInfo) {
                ...
                _each(axesInfo, function(i, info) {
                    ...
                    else if (!isDefinedUtils(correctValue))
                        correctValue = Math.abs((info.maxValue - info.minValue) / info.tickValues[mathFloor(info.tickValues.length / 2)])
                });
                if (!isDefinedUtils(correctValue))
                    return;
                _each(invalidAxisInfo, function(i, info) {
                    var correctedTick = info.tickValues[0] * correctValue;
                    if (info.tickValues[0] > 0) {
